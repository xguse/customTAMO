...
class AlignAce:
    ...
    def __init__(self,file = '',width=10, args="",seed=''):
        ...
        if (file[-4:] == '.fsa' or '.fas') or (file.find('.fasta')>0):  #Are we reading an AlignACE file or Analyzing a Fasta file?
            self.fastafile = file
        else:
            self.outfile = file
        if self.fastafile:
...
class MetaAce:
    ...
    def __init__(self,fastafile = '', width=10, iterations = 5, gcback=0.38):
        self.results = []
        self.lines   = []  #AD 02-23-09 to store each iteration's AlignAce file
    ...
    def _execute(self,fastafile, width, iterations):
        ...
        for i in range(iterations):
            ...
            #print A.lines[6],
            self.lines.extend(A.lines) #AD 02-23-09 to store each iteration's AlignAce file
